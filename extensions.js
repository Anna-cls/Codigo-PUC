...
renderOccurrenceCards = function() {
  ...
  const sharedOccurrences = JSON.parse(localStorage.getItem('occurrences')) || [];
  ...
  sharedOccurrences.forEach(occ => {
    ...
    if (!existingCard) {
      ...
      card.innerHTML = `
        ...
        <small>Registrado em ${occ.date} por ${occ.createdBy || 'Usuário'}</small>
...
handleOccurrenceSubmit = function(event) {
  ...
  if (title && category && description) {
    ...
    const sharedOccurrences = JSON.parse(localStorage.getItem('occurrences')) || [];
    const newId = sharedOccurrences.length ? sharedOccurrences[sharedOccurrences.length - 1].id + 1 : 1;
    const now = new Date();
